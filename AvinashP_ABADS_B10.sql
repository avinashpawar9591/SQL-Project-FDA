...
SELECT YEAR(DocDate) AS ApprovalYear, count(ActionType) AS NumberofDrugsApproved FROM appdoc WHERE ActionType = 'AP' GROUP BY YEAR(DocDate) ORDER BY ApprovalYear;
...
SELECT YEAR(appdoc.DocDate) AS ApprovalYear, application.SponsorApplicant, COUNT(*) AS NumberOfApprovals
FROM appdoc JOIN application ON appdoc.ApplNo = application.ApplNo
WHERE appdoc.ActionType = 'AP' 
GROUP BY YEAR(appdoc.DocDate), application.SponsorApplicant
...
FROM regactiondate LEFT JOIN application ON regactiondate.ApplNo = application.ApplNo
...
SELECT p.TECode, r.ActionType, YEAR(r.ActionDate) AS ApprovalYear, COUNT(r.ApplNo) AS TotalApprovals
